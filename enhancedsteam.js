...
function add_info(node, appid) {
	...
	if (v) { highlight_owned(node); return; }
	
	var c = getValue(appid+"c");
	if (c) { highlight_coupon(node); return }
	
	var w = getValue(appid+"w");
	if (w) { highlight_wishlist(node); return }
	
	var i = getValue(appid+"i");
	if (i) { highlight_inv(node); return }
