...
function add_app_badge_progress(appid) {
	if ($(".icon").find('img[src$="/ico_cards.png"]').length > 0) {
		$(".communitylink .block_content:last").append("<div class='rule'></div><div class='block_content_inner es_badge_block'><link rel='stylesheet' type='text/css' href='http://cdn.steamcommunity.com/public/css/skin_1/badges.css'><div class='es_badge_progress'></div><div class='es_foil_badge_progress'></div></div><div style=\"clear: both\"></div>");
		$(".es_badge_progress").load("http://steamcommunity.com/my/gamecards/" + appid + "/ .badge_current", function(responseText) {
			if ($(responseText).find(".friendPlayerLevelNum").length != 1) {
				...
				if(card_num_owned>0){
					$(".es_badge_progress").after("<div class='block_content_inner' style='padding-top: 2px; padding-bottom: 2px; color: #5491cf;'>" + card_num_owned + " of " + card_num_total + " cards owned</div>");
				}
				$(".es_badge_progress").after("<div class='block_content_inner' style='padding-top: 10px; padding-bottom: 2px; color: #5491cf;'>" + $(responseText).find(".progress_info_bold").text() + "</div>");
		...
		$(".es_foil_badge_progress").load("http://steamcommunity.com/my/gamecards/" + appid + "/?border=1 .badge_current", function(responseText) {
			if ($(responseText).find(".friendPlayerLevelNum").length != 1) {
				if ($(responseText).find(".badge_empty_circle").length != 1) {
					var card_num_owned = $(responseText).find(".badge_detail_tasks .owned").length;
					var card_num_total = $(responseText).find(".badge_detail_tasks .badge_card_set_card").length;
					...
					if(card_num_owned>0){
						$(".es_foil_badge_progress").after("<div class='block_content_inner' style='padding-top: 2px; padding-bottom: 2px; color: #5491cf;'>" + card_num_owned + " of " + card_num_total + " cards owned</div>");
					}
					...
					$(".es_foil_badge_progress .badge_empty_right").append("<div style=\"clear: both\"></div>");
				} else {
