...
let FriendsPageClass = (function(){
    ...
    FriendsPageClass.prototype.addPoster = function() {
        ...
        let min = 1;
        let max = 250; // this should avoid any mass actions by potential bad actors
        let warning = Localization.str.friends_commenter_warning.replace("__min__", min).replace("__max__", max);
        ExtensionLayer.runInPageContext(`function() {
            ...
            $J("#comment_submit").click(() => {
                ...
                if (total < ${min} || total > ${max} || msg.length === 0) {
                    ShowAlertDialog("${Localization.str.alert}", "${warning}");
                    return;
                }

                // this should further avoid any mass actions by potential bad actors
