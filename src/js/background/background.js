...
class SteamStore extends Api {
    ...
    static async fetchPackage({"key": subid}) {
        const data = await SteamStore.getEndpoint("/api/packagedetails/", {"packageids": subid});
    ...
    static async wishlistAdd(appid) {
        let res;
        const sessionid = await SteamStore.sessionId();
    ...
    static async currencyFromWallet() {
        const html = await SteamStore.getPage("/steamaccount/addfunds");
    ...
    static async currencyFromApp() {
        const html = await SteamStore.getPage("/app/220");
        const dummyPage = HTMLParser.htmlToDOM(html);
    ...
    static async wishlists(path) {
        const html = await SteamStore.getPage(`/wishlist${path}`);
    ...
    static async purchaseDate({"params": lang}) {
        ...
        const html = await SteamStore.getPage("/account/licenses/", {"l": lang});
        const dummyPage = HTMLParser.htmlToDOM(html);
    ...
    static async dynamicStore() {
        const {rgOwnedApps, rgOwnedPackages, rgIgnoredApps, rgWishlist} = await SteamStore.getEndpoint("dynamicstore/userdata");
...
class SteamCommunity extends Api {
    ...
    static async items() { // context#6, community items
        ...
        const data = await SteamCommunity.getInventory(6);
        if (data) {
    ...
    static async fetchWorkshopFileSize({"key": id}) {
        ...
        const res = await SteamCommunity.getPage("/sharedfiles/filedetails/", {id});
    ...
    static async fetchReviews({"key": steamId, "params": {reviewCount}}) {
        ...
        for (let p = 1; p <= Math.ceil(reviewCount / pageCount); p++) {
            const doc = parser.parseFromString(await SteamCommunity.getPage(`${steamId}/recommended`, {p}), "text/html");
