...
export class AddToCart {
    ...
    static async post(formEl, onWishlist, addToCartEl) {
        ...
        try {
            response = await RequestData.post("https://store.steampowered.com/cart/", new FormData(formEl), {}, "none");
        } catch (err) {
        ...
        const firstTime = !SyncedStorage.has("addtocart_no_redirect");
        ...
        if (onWishlist) {
            // On wishlists, show an alert instead of reloading
            if (!firstTime) {
                Page.runInPageContext((str) => {
                    window.SteamFacade.showAlertDialog(str.title, str.desc);
                }, [Localization.str.addtocart_dialog]);
            }

            // Steam doesn't indicate the item has been added to Cart, so disable the button for now
            addToCartEl.classList.add("btn_disabled");
